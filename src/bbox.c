...
struct boundingBox *computeBoundingBox(int *polys, int count) {
	...
	for (int i = 0; i < count; i++) {
		for (int j = 0; j < 3; j++) {
			minPoint = minVector(&minPoint, &vertexArray[polygonArray[polys[i]].vertexIndex[j]]);
			maxPoint = maxVector(&maxPoint, &vertexArray[polygonArray[polys[i]].vertexIndex[j]]);
		}
	}
	...
	for (int i = 0; i < count; i++) {
		for (int j = 0; j < 3; j++) {
			struct vector fromCenter = subtractVectors(&vertexArray[polygonArray[polys[i]].vertexIndex[j]], &center);

...
static int load_env_map(const file_data data, struct texture *out) {
	if (!out) return 1;
	float *new = stbi_loadf_from_memory(data, (int)v_arr_len(data), (int *)&out->width, (int *)&out->height, (int *)&out->channels, 0);
	if (!new) {
...
static int load_qoi_from_buffer(const file_data data, struct texture *out) {
	if (!out) return 1;
	qoi_desc desc;
	void *decoded_data = qoi_decode(data, v_arr_len(data), &desc, 3);
	if (!decoded_data) {
...
static int load_texture_from_buffer(const file_data data, struct texture *out) {
	if (!out) return 1;
	unsigned char *new = stbi_load_from_memory(data, v_arr_len(data), (int *)&out->width, (int *)&out->height, (int *)&out->channels, 0);
	if (!new) {
		logr(warning, "Failed to decode texture from memory buffer of size %zu. Reason: \"%s\"\n", v_arr_len(data), stbi_failure_reason());
		return 1;
	}
	if (out->data.byte_p) free(out->data.byte_p);
	out->data.byte_p = new;
...
int load_texture(const char *path, file_data data, struct texture *out) {
	...
	int ret = 0;
	if (stbi_is_hdr_from_memory(data, v_arr_len(data))) {
	...
	char b0[64];
	char b1[64];
	logr(debug, "Loaded texture %s, %s => %s\n", path, human_file_size(v_arr_len(data), b0), human_file_size(raw_bytes, b1));
	return 0;
error:
